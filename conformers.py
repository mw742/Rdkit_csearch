  ...
  * http://pubs.acs.org/doi/full/10.1021/ci2004658
  ...
  def get_conformer_energies(self, mol):
    ...
    for conf in mol.GetConformers():
      ...
      if energy < lowest_energy_value:
          print(lowest_energy_value)
          lowest_energy_value = energy
          lowest_confID = conf.GetId()
          print(lowest_confID)
      else:
        pass
...
#######################################################################
## For each mol read, do the cs
#######################################################################
for inchi in inchi_list:
    mol = Chem.MolFromInchi(inchi)
    file_number = inchi_list.index(inchi)
    res = ConformerGenerator()
